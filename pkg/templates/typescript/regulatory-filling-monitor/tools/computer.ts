...
import type { ActionParams, BaseAnthropicTool, ToolResult } from './types/computer.ts';
import { Action, ToolError } from './types/computer.ts';
import { ActionValidator } from './utils/validator.ts';
...
export class ComputerTool implements BaseAnthropicTool {
  ...
  async call(params: ActionParams): Promise<ToolResult> {
    ...
    if (action === Action.SCROLL) {
      ...
      let delta_x = 0;
      let delta_y = 0;
      // Each scroll_amount unit = ~25 pixels for fine-grained control when extracting data from tables
      // This allows the model to scroll just a few rows at a time
      const scrollDelta = (scrollAmountValue ?? 1) * 5;
